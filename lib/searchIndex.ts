...
export function buildSearchIndex(t: TFunc): SearchResult[] {
  ...
  items.push(
    ...
    item({
      ...
      subtitle: t("hospitalsSub", { defaultValue: "Hospitals & pharmacies near holy sites" }),
      icon: "medical",
      category: "Services",
      action: "navigate",
      target: "/maps/hospital-makkah",
      keywords: kw("nahdi", "النهدي", "pharmacy", "pharmacies", "ibrahim al khaleel", "haram pharmacy"),
      boost: 12,
    }),
    item({
      id: "makkah-places",
  ...
  for (const place of MAKKAH_PLACES) {
    ...
    items.push(
      item({
        ...
        icon: place.id.includes("museum") ? "mosque" : "bag",
        category: "Maps",
        action: "navigate",
